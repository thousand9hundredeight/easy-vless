...
Набор скриптов **идемпотентен**: вы можете запускать `easy-install.sh` несколько раз. При каждом запуске старая установка VLESS удаляется, и контейнер пересоздаётся с новыми ключами и параметрами, без затрагивания уже работающих сервисов (например, MTProto через ваш nginx) [web:113][web:422].
...
- Основной контейнер Xray‑core `vless-reality` публикуется на отдельном порту, например `:1443/tcp` (или `:2443/tcp`), чтобы не мешать nginx‑сервису MTProto на `:443` [web:417][web:421].
...
В конце будет сгенерирована готовая к импорту клиентская ссылка `vless://...` и сохранена в `/root/vless-credentials.txt` [web:140][web:415].
...
- Скрипты не лезут в логику друг друга, а считывают все настройки из `.env` и из начала `easy-install.sh` [web:113].
...
Все вспомогательные скрипты будут читать одни и те же значения из `.env` [web:113][web:415].
...
3. Учётные данные:
   ...
   Права на файл установлены в `600` для безопасности [web:140].
...
5. Мониторинг:
   ...
   - `ports` (список открытых портов) [web:415].
...
Это единственная строка, которую нужно импортировать в клиенте (Android/iOS/Windows) [web:140].
...
- Не трогает `nginx` для MTProto, уже работающие контейнеры других сервисов и tmux‑сессии `vpnmon`, если их установщики не запускаются повторно [web:424].
...
## Важно: VLESS и MTProto на одном сервере

- nginx на порту `443` обслуживает **MTProto** и не включён в VLESS‑цепочку (`VLESS_USE_NGINX=false`).  
- VLESS‑Reality слушает другой порт (`1443` или `2443`), чтобы не конфликтовать с nginx.
- Если вы хотите со временем перейти на nginx‑прокси поверх VLESS, это можно сделать отдельно, но в данном README оставлен «чистый» сценарий без пересечения VLESS‑Traffic и nginx‑цепочки [web:418][web:419].
